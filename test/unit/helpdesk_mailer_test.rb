...
class HelpdeskMailerTest < ActiveSupport::TestCase
  ...
  test "Should add issue and contact" do
    ...
    issue = submit_email('new_issue_new_contact.eml', :issue => {:project_id => 'ecookbook'})
  ...
  test "Should add duplicated contact" do
    ...
    # This email contains: 'Project: onlinestore'
    issue = submit_email('ticket_without_name.eml', :issue => {:project_id => 'ecookbook'})
    assert_equal Issue, issue.class
    assert !issue.new_record?
    contact = issue.customer
  ...
  test "Should assign ticket to group" do
    ...
    group = Group.last
    ContactsSetting[:helpdesk_answer_from, Project.find('ecookbook').id] = 'test@email.from'
    issue = submit_email('new_issue_new_contact.eml', :issue => {:project_id => 'ecookbook', :assigned_to_id => group.id})
    assert_equal Issue, issue.class
    assert !issue.new_record?
    issue.reload
  ...
  test "Should assign ticket to user" do
    ...
    user = User.last
    ContactsSetting[:helpdesk_answer_from, Project.find('ecookbook').id] = 'test@email.from'
    issue = submit_email('new_issue_new_contact.eml', :issue => {:project_id => 'ecookbook', :assigned_to_id => user.id})
    assert_equal Issue, issue.class
    assert !issue.new_record?
    issue.reload
  ...
  test "Should set author from redmine user" do
    ...
    issue = submit_email('ticket_from_redmine_user.eml', :issue => {:project_id => 'ecookbook', :assigned_to_id => user.id})
    assert_equal Issue, issue.class
    assert !issue.new_record?
    issue.reload
  ...
  test "Should recieve ticket without to address" do
    ActionMailer::Base.deliveries.clear
    issue = submit_email('ticket_with_empty_to.eml', :issue => {:project_id => 'ecookbook'})
    assert_equal Issue, issue.class
    assert !issue.new_record?
    issue.reload
    assert_equal issue, Issue.find_by_subject('New support issue from email')
  ...
  test "Should not recieve ticket without from address" do
    ActionMailer::Base.deliveries.clear
    issue = submit_email('ticket_with_empty_from.eml', :issue => {:project_id => 'ecookbook'})
  ...
  test "Should add contact with bad name" do
    ...
    ContactsSetting[:helpdesk_answer_from, Project.find('ecookbook').id] = 'test@email.from'
    ContactsSetting[:helpdesk_send_notification, Project.find('ecookbook').id] = 1
    issue = submit_email('new_contact_bad_name.eml', :issue => {:project_id => 'ecookbook'})
    assert_equal Issue, issue.class
    assert !issue.new_record?
    issue.reload
    assert_equal issue, Issue.find_by_subject('New support issue from email')
  ...
  test "Should add contact with unicode name" do
    ...
    ContactsSetting[:helpdesk_answer_from, Project.find('ecookbook').id] = 'test@email.from'
    ContactsSetting[:helpdesk_send_notification, Project.find('ecookbook').id] = 1
    issue = submit_email('new_contact_unicode_name.eml', :issue => {:project_id => 'ecookbook'})
    assert_equal Issue, issue.class
    assert !issue.new_record?
    issue.reload
    assert_equal issue, Issue.find_by_subject('New support issue from email')
  ...
  test "Should add new issue and contact with encoded name" do
    ...
    ContactsSetting[:helpdesk_answer_from, Project.find('ecookbook').id] = 'test@email.from'
    ContactsSetting[:helpdesk_send_notification, Project.find('ecookbook').id] = 1
    issue = submit_email('new_issue_new_contact_encode.eml', :issue => {:project_id => 'ecookbook'})
    assert_equal Issue, issue.class
    assert !issue.new_record?
    issue.reload
  ...
  test "Should add new issue from html only body" do
    ActionMailer::Base.deliveries.clear
    # This email contains: 'Project: onlinestore'
    issue = submit_email('ticket_html_only.eml', :issue => {:project_id => 'ecookbook'})
    assert_equal Issue, issue.class
    assert !issue.new_record?
    issue.reload
  ...
  test "Should add new issue to contact without subject" do
    ActionMailer::Base.deliveries.clear
    # This email contains: 'Project: onlinestore'
    issue = submit_email('new_issue_no_subject.eml', :issue => {:project_id => 'ecookbook'})
    assert_equal Issue, issue.class
  ...
  test "Should add issue with rus attachment" do
    ...
    ContactsSetting[:helpdesk_answer_from, Project.find('ecookbook').id] = 'test@email.from'
    ContactsSetting[:helpdesk_send_notification, Project.find('ecookbook').id] = 1
    issue = submit_email('ticket_with_rus_attachment.eml', :issue => {:project_id => 'ecookbook'})
    assert_equal Issue, issue.class
    assert !issue.new_record?
    issue.reload
    # assert_equal issue, Issue.find_by_subject('New support issue from email')
  ...
  test "Should add issue and contact ru" do
    ...
    ContactsSetting[:helpdesk_answer_from, Project.find('ecookbook').id] = 'test@email.from'
    ContactsSetting[:helpdesk_send_notification, Project.find('ecookbook').id] = 1
    issue = submit_email('new_issue_new_contact_ru.eml', :issue => {:project_id => 'ecookbook'})
    assert_equal Issue, issue.class
    assert !issue.new_record?
    issue.reload
    # assert_equal issue, Issue.find_by_subject('New support issue from email')
  ...
  test "Should add issue and contact ru 2" do
    ...
    ContactsSetting[:helpdesk_answer_from, Project.find('ecookbook').id] = 'test@email.from'
    ContactsSetting[:helpdesk_send_notification, Project.find('ecookbook').id] = 1
    issue = submit_email('new_issue_new_contact_ru_2.eml', :issue => {:project_id => 'ecookbook'})
    assert_equal Issue, issue.class
    assert !issue.new_record?
    issue.reload
    # assert_equal issue, Issue.find_by_subject('New support issue from email')
  ...
  test "Should add issue and contact ru 4" do
    ActionMailer::Base.deliveries.clear
    # This email contains: 'Project: onlinestore'
    issue = submit_email('new_issue_new_contact_ru_4.eml', :issue => {:project_id => 'ecookbook'})
    issue.reload
    assert_equal Issue, issue.class
    assert_equal '(no subject)', issue.subject
    attachment = issue.attachments.last
  ...
  test "Should add issue and contact ru 5" do
    ActionMailer::Base.deliveries.clear
    # This email contains: 'Project: onlinestore'
    issue = submit_email('new_issue_new_contact_ru_5.eml', :issue => {:project_id => 'ecookbook'})
    issue.reload
    assert_equal Issue, issue.class
  ...
  def test_should_create_from_quoted_body
    ActionMailer::Base.deliveries.clear
    # This email contains: 'Project: onlinestore'
    issue = submit_email('ticket_with_quotes.eml', :issue => {:project_id => 'ecookbook'})
    issue.reload
  ...
  test "Should add issue and contact with French symbols" do
    ActionMailer::Base.deliveries.clear
    # This email contains: 'Project: onlinestore'
    issue = submit_email('new_issue_french.eml', :issue => {:project_id => 'ecookbook'})
    issue.reload
    assert_equal Issue, issue.class
  ...
  test "Should add issue to contact" do
    ...
    # This email contains: 'Project: onlinestore'
    ContactsSetting[:helpdesk_send_notification, Project.find('ecookbook').id] = 1
    issue = submit_email('new_issue_to_contact.eml', :issue => {:project_id => 'ecookbook'})
    assert_equal Issue, issue.class
    assert !issue.new_record?
    issue.reload
  ...
  test "Should add project to existing contact" do
    ...
    issue = submit_email('new_issue_to_contact.eml', :issue => {:project_id => 'onlinestore'})
    assert_equal Issue, issue.class
    assert !issue.new_record?
    issue.reload
  ...
  test "Should reply to issue to contact" do
    ...
    journal = submit_email('reply_from_contact.eml', :issue => {:project_id => 'ecookbook'}, :reopen_status => 'Feedback')
  ...
  test "Should add cc to issue" do
    ...
    issue = submit_email('ticket_with_cc.eml', :issue => {:project_id => 'ecookbook'})
    assert_equal Issue, issue.class

    assert_equal "cc@somenet.foo,marat.aminov@somenet.foo,ivan@somenet.foo", issue.helpdesk_ticket.cc_address
    assert_equal "Марат Аминов", issue.contacts[2].name
  end
  ...
  test "Should reply to issue to contact with attachment" do
    ...
    # This email contains: 'Project: onlinestore'
    journal = submit_email('reply_with_attachment.eml', :issue => {:project_id => 'ecookbook'}, :reopen_status => 'Feedback')
    assert_equal Journal, journal.class
    assert !journal.new_record?
  ...
  test "Should ignore auto answer" do
    ...
    journal = submit_email('auto_answer.eml', :issue => {:project_id => 'ecookbook'}, :reopen_status => 'Feedback')
  ...
  #   ActionMailer::Base.deliveries.clear
  #   # This email contains: 'Project: onlinestore'
  #   issue = submit_email('ticket_with_leading_spaces_to.eml', :issue => {:project_id => 'ecookbook'})
  #   assert_equal Issue, issue.class
  #   assert !issue.new_record?
  #   issue.reload
  ...
  test "Should find issue by in_reply_to" do
    ...
    journal.save!
    response_journal = submit_email('ticket_with_in_reply_to.eml', :issue => {:project_id => 'ecookbook'}, :reopen_status => 'Feedback')
  ...
  test "Should add journal with empty status settings" do
    ...
    journal.journal_message = JournalMessage.new(:contact => contact, 
                                                 ...
                                                 :message_id => '123456789@mail.com')
    journal.save!
    response_journal = submit_email('ticket_with_in_reply_to.eml', :issue => {:project_id => 'ecookbook'}, :reopen_status => "")
    assert_equal Journal, response_journal.class
    assert_equal response_journal.issue, journal.issue
