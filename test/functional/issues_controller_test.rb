...
class IssuesControllerTest < ActionController::TestCase
  ...
  def test_should_send_note
    ...
    assert_equal Issue.find(1).contacts.last.emails.first, mail.to.first
  end
  ...
  def test_should_send_note_with_bcc
    ...
    assert_mail_body_match "Hello, Ivan\r\n Bye, #{user.firstname}", mail
    assert_equal Issue.find(1).contacts.last.emails.first, mail.to.first
    assert_equal 'mail1@mail.com', mail.bcc.first
  end

  def test_should_not_send_note_to_bcc
    issue = Issue.find(1)
    contact = Contact.find(1)
    user = User.find(1)
    issue.helpdesk_ticket = HelpdeskTicket.new(:customer => contact, 
    ...
    assert_mail_body_match "Hello, Ivan\r\n Bye, #{user.firstname}", mail
    assert_equal Issue.find(1).contacts.last.emails.first, mail.to.first
    assert_equal 'mail3@mail.com', mail.cc.first
    assert_nil mail.bcc.first
  end  
  ...
  def test_should_not_create_ticket_with_empty_customer
    ...
    assert_no_difference 'HelpdeskTicket.count' do
      post :create,
           :helpdesk_ticket => {:source => "0", :ticket_date => "2013-01-01 01:01:01 +0400"},
           :issue => {:tracker_id => 3, :subject => "test", :status_id => 2, :priority_id => 5},
           :project_id => 1
    end
    assert_redirected_to :controller => 'issues', :action => 'show', :id => Issue.last.id
