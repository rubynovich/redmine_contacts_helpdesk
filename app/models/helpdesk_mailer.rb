class HelpdeskMailer < MailHandler
  ...
  def issue_response(contact, journal, params)
    ...
    response_message = ''
    response_message << self.class.apply_macro(HelpdeskSettings[:helpdesk_emails_header, journal.issue.project], contact, journal.issue, journal.user) + "\n\n" unless HelpdeskSettings[:helpdesk_emails_header, journal.issue.project].blank?
    response_message << self.class.apply_macro(journal.notes, contact, journal.issue, journal.user)
    response_message << "\n\n" + self.class.apply_macro(HelpdeskSettings[:helpdesk_emails_footer, journal.issue.project], contact, journal.issue, journal.user)  unless HelpdeskSettings[:helpdesk_emails_footer, journal.issue.project].blank?
    response_message << ''
    
    raise MissingInformation.new("Contact #{contact.name} should have mail") if contact.email.blank?
    raise MissingInformation.new("Message shouldn't be blank") if response_message.blank? 
    ...
    @email_body = textile(response_message).html_safe
    ...
    mail :from => from_address,
      ...
      format.text { render :text => response_message }
  ...
  def receive(email)
    @email = email
    if !target_project.module_enabled?(:contacts_module) || !target_project.module_enabled?(:issue_tracking)
  ...
  def auto_answer(contact, issue)
    ...
    @email_body = textile(confirmation_body).html_safe
    ...
    mail :from => from_address,
      ...
      format.text { render :text => confirmation_body }
      format.html { render 'email_layout' } unless RedmineHelpdesk.settings[:plain_text_mail]
    end
  ...
  def self.apply_macro(text, contact, issue, journal_user=nil)
    ...
    text = text.gsub(/%%DATE%%|\{%date%\}/, Date.today.to_s)
