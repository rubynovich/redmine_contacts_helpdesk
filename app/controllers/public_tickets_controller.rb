class PublicTicketsController < ApplicationController
  ...
  def show
    @previous_tickets = @ticket.customer.tickets.where(["#{Issue.table_name}.id <> ?", @issue.id]).order("#{Issue.table_name}.status_id, #{Issue.table_name}.due_date DESC, #{Issue.table_name}.updated_on DESC").first(11)

    @total_spent_hours = @ticket.customer.tickets.map.sum(&:total_spent_hours)
