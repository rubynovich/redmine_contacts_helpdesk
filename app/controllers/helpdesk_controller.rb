class HelpdeskController < ApplicationController
  ...
  def create_ticket
    raise Exception, "Param 'issue' and 'contact' should be set" unless params[:issue] || params[:contact]
    @issue = Issue.new
    ...
    raise Exception, "Contact should have email address" unless params[:contact] || params[:contact][:email]

    contacts = Contact.find_by_emails([params[:contact][:email]])
    @contact = contacts.first if contacts.any?
    ...
    respond_to do |format|
      format.api { render :text => (params[:format] == "xml" ? @issue.to_xml : @issue.to_json), :status => :created} 
    end
